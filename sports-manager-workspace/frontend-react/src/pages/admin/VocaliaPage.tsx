...
import { useVocaliaSocket } from '../../hooks/useVocaliaSocket';
import {
  fmt,
  EventLogGrid,
  FinesPanel,
  EventButtons,
} from '../../components/vocalia/VocaliaComponents';
...
export default function VocaliaPage() {
  ...
  const startMatch = () =>
    socketRef.current?.emit('start_match', { matchId, halfDurationMinutes: halfDuration });
  ...
  const saveColors = () => {
    socketRef.current?.emit('set_colors', { matchId, homeColor, awayColor });
    setShowColors(false);
  };
  ...
  const openEventModal = (type: EventType, teamId: string) => {
    if (type === 'FOUL') {
      socketRef.current?.emit('register_event', { matchId: matchId!, teamId, eventType: 'FOUL' });
      return;
    }
    setSelectedPlayerId('');
    setSelectedPlayerOutId('');
    setEventModal({ type, teamId });
  };
  ...
  const openFineModal = (teamId: string) => {
    setFineTeamId(teamId);
    setFineReason('Llegada tarde');
    setFineAmount(tournament?.lateFine ?? 10000);
  ...
  const saveFine = async () => {
    ...
    try {
      await finesApi.create({
        ...
        amount: fineAmount,
        reason: fineReason,
        half: timer.currentHalf,
      });
      await fetchFines();
      setFineSuccess(true);
      setTimeout(() => { setFineModal(false); setFineSuccess(false); }, 1200);
    } catch (e) {
      console.error('Error saving fine', e);
    } finally {
      setFineSaving(false);
    }
  };
  ...
  const downloadActa = async () => {
    try {
      const blob = await matchesApi.getActa(matchId!);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `acta-${matchId}.pdf`;
      a.click();
  ...
  const modalPlayers: Player[] =
    eventModal?.teamId === match?.homeTeamId ? homePlayers : awayPlayers;
  ...
  const getPlayerName = (id: string | null) =>
    id ? (allPlayers.find((p) => p.id === id)?.name ?? null) : null;

  const expelledPlayer = expulsionAlert
    ? allPlayers.find((p) => p.id === expulsionAlert.playerId)
    : null;
  const expelledTeam =
    expulsionAlert?.teamId === match?.homeTeamId ? homeTeam : awayTeam;
  ...
  const teamName = (teamId: string) =>
    teamId === match?.homeTeamId ? homeTeam?.name : awayTeam?.name;

  const eventLog = [...events]
    .filter((e) => e.eventType !== 'SUBSTITUTION')
    .sort((a, b) => a.minute - b.minute);

  const statusLabel = matchClosed
    ? 'Finalizado'
    : isInProgress
    ? halfEnded
      ? '¡Tiempo!'
  ...
  const pendingForTeam = (tid: string) => {
    const finesTotal = fines
      .filter((f) => f.teamId === tid && f.status === 'PENDING')
  ...
  return (
    <div className="min-h-dvh bg-gray-950 text-white flex flex-col">
          ...
          >
            ...
            <div>
              <p className="text-sm font-bold text-red-300">
                ¡Expulsión automática — min. {expulsionAlert.minute}!
              </p>
              <p className="text-xs text-red-400">
                {expelledPlayer
                  ? `#${expelledPlayer.dorsal} ${expelledPlayer.name}`
                  : 'Jugador'}{' '}
                · {expelledTeam?.name ?? 'Equipo'} · 2 amarillas
              </p>
            </div>
          </motion.div>
        )}
        ...
        <div className="flex items-center gap-2">
          {connected ? (
            <>
              ...
              <span className="text-green-400 font-medium text-xs sm:text-sm">En línea</span>
            </>
          ) : (
            <>
              <WifiOff className="w-4 h-4 text-red-400" />
              <span className="text-red-400 font-medium text-xs sm:text-sm">Desconectado</span>
            </>
          )}
        </div>
      </header>
      ...
      <div className="md:hidden flex flex-col flex-1 min-h-0">
        ...
        <div className="shrink-0 flex items-start px-5 pt-5 pb-4 gap-2">
          ...
          <div className="flex flex-col items-center shrink-0 px-2 pt-3 gap-1">
            ...
            {halfEnded && isInProgress && (
              <span className="text-[9px] font-bold text-amber-400 animate-pulse mt-0.5">
                ¡Tiempo!
              </span>
            )}
          </div>
          ...
          <div className="flex-1 flex flex-col gap-2 items-end min-w-0">
            ...
            <span className="text-sm font-semibold text-gray-200 text-right leading-tight truncate">
        ...
        {/* ── Pill tabs ── */}
        <div
          className="shrink-0 flex gap-2 px-4 py-3 border-y border-gray-800/50 overflow-x-auto"
        ...
        >
          {(
            [
              ...
              {
                id: 'fines' as MobileTab,
                label: `Multas${fines.length > 0 ? ` (${fines.length})` : ''}`,
              },
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          {mobileTab === 'match' && (
            <div className="p-4 pb-8 space-y-5">
              {!matchClosed && (isScheduled || half1Ended) && (
                <div className="space-y-2">
                  {isScheduled && (
                    <button
                      onClick={startMatch}
                      className="w-full bg-green-600 hover:bg-green-500 active:bg-green-700 text-white font-bold py-4 min-h-[52px] rounded-2xl transition-colors text-base"
                    >
                      ▶ Iniciar Partido
                    </button>
                  )}
                  {half1Ended && (
                    <button
                      ...
                      className="w-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white font-bold py-4 min-h-[52px] rounded-2xl transition-colors text-base"
                    >
                      ▶ Iniciar 2do Tiempo
                    </button>
                  )}
              ...
              {matchClosed && (
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 max-w-xs">
                    El acta fue generada automáticamente.
                  </p>
                  <button
                    onClick={downloadActa}
                    className="mt-2 bg-indigo-900 hover:bg-indigo-800 border border-indigo-700 text-indigo-300 hover:text-white font-semibold text-sm py-3 min-h-[44px] px-6 rounded-xl transition-all"
                  >
                    Descargar Acta PDF
                  </button>
                </motion.div>
              ...
              {matchClosed &&
                (pendingForTeam(match.homeTeamId) > 0 ||
                  pendingForTeam(match.awayTeamId) > 0) && (
          ...
          {mobileTab === 'fines' && (
            <div className="p-4 pb-8">
              <FinesPanel
                half1Fines={half1Fines}
                half2Fines={half2Fines}
                half1Total={half1Total}
                half2Total={half2Total}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold">
                    {matchClosed ? 'Partido cerrado' : isInProgress ? 'En curso' : 'Programado'}
                  </span>
                </div>
              </div>
              ...
              {!matchClosed && (
                <div className="flex flex-col gap-2">
                  {isScheduled && (
                    <button
                      onClick={startMatch}
                      className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 min-h-[48px] rounded-xl transition-colors text-sm"
                    >
                      ▶ Iniciar Partido
                    </button>
                  )}
                  {half1Ended && (
                    <button
                      onClick={startSecondHalf}
                      className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 min-h-[48px] rounded-xl transition-colors text-sm"
                    >
                      ▶ Iniciar 2do Tiempo
                    </button>
                  )}
                  {isInProgress && (
                    <button
                      onClick={closeMatch}
                      className="w-full bg-gray-800 hover:bg-red-900 border border-gray-700 hover:border-red-700 text-gray-400 hover:text-red-300 font-semibold py-3 min-h-[44px] rounded-xl transition-all text-sm mt-2"
                    >
                      Cerrar partido
                    </button>
                  )}
              ...
              <div className="border-t border-gray-800 pt-4">
                ...
                <AnimatePresence>
                  {showColors && (
                    ...
                    >
                      {[
                        { label: homeTeam?.name ?? 'Local', color: homeColor, set: setHomeColor },
                        { label: awayTeam?.name ?? 'Visitante', color: awayColor, set: setAwayColor },
                      ].map(({ label, color, set }) => (
                        <div key={label}>
                      ...
                      <button
                        onClick={saveColors}
                        className="w-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold py-2.5 min-h-[44px] rounded-lg transition-colors"
                      >
                        Guardar colores
                      </button>
                    </motion.div>
          ...
          {!matchClosed && (
            <div className="flex flex-col gap-2">
              ...
              {isInProgress && (
                <button
                  onClick={closeMatch}
                  className="w-full bg-gray-800 hover:bg-red-900 border border-gray-700 hover:border-red-700 text-gray-400 hover:text-red-300 font-semibold py-2.5 rounded-xl transition-all text-sm mt-2"
                >
                  Cerrar partido
                </button>
              )}
            ...
            <AnimatePresence>
              {showColors && (
                ...
                >
                  ...
                  <button
                    onClick={saveColors}
                    className="w-full bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold py-2.5 rounded-lg transition-colors"
                  >
                    Guardar colores
                  </button>
                </motion.div>
        ...
        <div className="flex flex-col items-center pt-10 gap-8 px-6 overflow-y-auto">
          ...
          <div className="w-full max-w-sm">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1 flex flex-col items-center gap-2">
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-black border-4 shadow-lg overflow-hidden"
                  style={{
                    borderColor: homeColor,
                    color: homeColor,
                    backgroundColor: homeColor + '22',
                  }}
                >
                  {homeTeam?.logoUrl && !homeLogoError ? (
                    <img
                ...
                <p className="text-xs font-semibold text-gray-300 text-center leading-tight">
                  {homeTeam?.name ?? 'Local'}
                </p>
              </div>
              <div className="flex flex-col items-center">
                <div className="flex items-baseline gap-1 tabular-nums">
                  <span className="text-8xl font-black leading-none" style={{ color: homeColor }}>
                    {match.homeScore}
                  </span>
                  <span className="text-5xl font-thin text-gray-700 mx-1">:</span>
                  <span className="text-8xl font-black leading-none" style={{ color: awayColor }}>
                    {match.awayScore}
                  </span>
                </div>
              </div>
          ...
          {matchClosed && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              className="flex flex-col items-center gap-3 text-center mt-4 w-full max-w-sm"
            >
              <CheckCircle className="w-14 h-14 text-green-400" />
              <p className="text-lg font-bold">Partido finalizado</p>
              <p className="text-sm text-gray-500 max-w-xs">
                El acta fue generada y enviada por WhatsApp al delegado del equipo local.
              </p>
              <button
                onClick={downloadActa}
                className="mt-2 bg-indigo-900 hover:bg-indigo-800 border border-indigo-700 text-indigo-300 hover:text-white font-semibold text-sm py-3 px-6 rounded-xl transition-all"
              >
                Descargar Acta PDF
              </button>
              {(pendingForTeam(match.homeTeamId) > 0 ||
                ...
                <div className="w-full border-t border-gray-800 pt-4 mt-2 space-y-2">
                  <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest text-left">
                    Pagos pendientes
                  </p>
                  {[
                    { team: homeTeam, id: match.homeTeamId },
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-2 mt-1.5">
                      {[
                        { id: match.homeTeamId, name: homeTeam?.name ?? 'Local' },
                        { id: match.awayTeamId, name: awayTeam?.name ?? 'Visitante' },
                      ].map(({ id, name }) => (
                        <button
                          key={id}
                          onClick={() => setFineTeamId(id)}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (['Llegada tarde', 'Equipo incompleto'].includes(e.target.value))
                          setFineAmount(tournament?.lateFine ?? 10000);
                      }}
                    ...
                    >
                      ...
                      <option>Equipo incompleto</option>
                      <option>Conducta del banco</option>
                      <option>Juego brusco</option>
                      <option>Otro</option>
                    </select>
                  </div>
                  <div>
                  ...
                  <div className="flex gap-3 pt-1">
                    ...
                    <button
                      onClick={saveFine}
                      disabled={fineSaving || !fineTeamId || fineAmount <= 0}
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs text-gray-500 font-medium">
                    {eventModal.type === 'SUBSTITUTION' ? 'Jugador que entra' : 'Jugador'}
                  </label>
                  <select
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button
                    ...
                    className="flex-1 bg-indigo-600 hover:bg-indigo-500 py-3 min-h-[44px] rounded-xl text-sm font-bold transition-colors"
                  >
                    Registrar
                  </button>
                </div>
